...
function readStoredAuthToken(): boolean | null {
...
export default function Home() {
  ...
  const [hasAuthToken, setHasAuthToken] = useState<boolean | null>(() => readStoredAuthToken())
  const { isMobile, isSmallScreen, isClient } = useMobileDetection()

  const checkStoredAuthToken = useCallback(() => {
    const tokenExists = readStoredAuthToken()
    setHasAuthToken(tokenExists)
    return Boolean(tokenExists)
  }, [])
  ...
  if (loading) {
    const shouldShowLanding = hasAuthToken !== true

    if (shouldShowLanding) {
      console.log('🚀 No stored session detected yet - showing marketing site while auth resolves')
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          <div className="mx-auto h-28 w-28 rounded-full border-4 border-purple-200 border-t-purple-600 animate-spin"></div>
          <p className="mt-4 text-gray-600 dark:text-gray-400">Loading your SplitSave account…</p>
